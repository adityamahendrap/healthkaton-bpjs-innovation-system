...
app.post("/driven-qna", async (req, res) => {
  ...
  // Function to check if the question is related to BPJS
  const isBPJSRelatedQuestion = () => {
    const bpjsKeywords = ["bpjs", "klaim", "kesehatan", "pelayanan"];
    const userQuestionLower = question.toLowerCase();

    for (const keyword of bpjsKeywords) {
  ...
  PERTANYAAN: ${question}`;
    ...
    const answer = `Maaf, pertanyaan Anda tidak terkait dengan BPJS. Saya hanya dapat memberikan informasi terkait BPJS`;
    res.json(answer);
  }
});
